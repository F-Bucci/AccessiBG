...
3. [**3. Testing del Frontend**](#3-testing-del-database)
...
| `Distributore` | `testGetAccessibileFalse()` | `12` | Test per verificare che un valore _false_ passato all'attributo `accessibile` rimanga _false_ |
...
`it.unibg.AccessiBG_Backend.dao` 
| **Componente testato** | **Nome Test**                   | **N.Test** | **Test Effettuato** |
|-|-|-|-|
| `SedeDAO`              | `testInsert()`                  | `1`        | Test per verificare l'inserimento di una nuova sede                                                         |
| `SedeDAO`              | `testInsertDuplicate()`         | `2`        | Test per verificare la gestione duplicati con `ON CONFLICT`                                                 |
| `SedeDAO`              | `testFindByFacolta()`           | `3`        | Test per verificare la ricerca di una sede per chiave naturale                                              |
| `SedeDAO`              | `testFindByFacoltaNotFound()`   | `4`        | Test per verificare che venga restituito `null` per sede inesistente                                        |
| `SedeDAO`              | `testFindAll()`                 | `5`        | Test per verificare il recupero di tutte le sedi                                                            |
| `SedeDAO`              | `testFindEdificiBySede()`       | `6`        | Test per verificare la query relazionale che trova gli edifici associati a una sede                         |
| `SedeDAO`              | `testFindParcheggioBySede()`    | `7`        | Test per verificare la query relazionale che trova i parcheggi associati a una sede                         |
| `SedeDAO`              | `testFindEdificiEmpty()`    | `8`        | Test per verificare che ti restituisca una lista vuota se si vanno ad inserire edificio con campi vuoti
...
Per garantire la copertura completa del software, sono stati adottati due livelli di astrazione distinti. Utilizzando insieme [Junit](#ljunit), si possono rilevare sia errori logici nel codice che errori di visualizzazione e interazione del utente con il browser. 
