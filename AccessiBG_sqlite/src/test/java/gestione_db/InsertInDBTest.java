...
public class InsertInDBTest {
    ...
    void testInserimentoCompleto() {
        ...
        assertEquals(6, sedeDAO.findAll().size());
        assertEquals(4, sedeDAO.findEdificiBySede("Sede di Dalmine"));
    ...
    @Test
    @DisplayName("Verifica relazioni FK")
    void testRelazioniForeignKey() {
        insertInDB.insRecordSedi();
        insertInDB.insRecordEdifici();
        ...
        // Verifica che tutti gli edifici puntino a sedi esistenti
        for (Edificio e : edificioDAO.findAll()) {
            assertNotNull(sedeDAO.findEdificiBySede(e.getFacolta()));
        }
    }
}